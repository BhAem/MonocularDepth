...
class Trainer:
    def __init__(self, options):
        ...
        self.models["encoder"] = networks.ResnetEncoder(
            self.opt.num_layers, self.opt.weights_init, use_aspp=self.opt.use_aspp)
        self.parameters_to_train += list(self.models["encoder"].parameters())

        self.models["depth"] = networks.DepthDecoder(
            self.models["encoder"].num_ch_enc, att_type=self.opt.att, use_frelu=self.opt.use_frelu, scales=self.opt.scales)
        self.parameters_to_train += list(self.models["depth"].parameters())

        if self.use_pose_net:
            if self.opt.pose_model_type == "separate_resnet":
                self.models["pose_encoder"] = networks.PoseResnetEncoder(
                    self.opt.num_layers,
        ...
        fpath = os.path.join(os.path.dirname(__file__), "splits", self.opt.split, "{}_files.txt")  # 数据集的路径
        # 1. line搞不懂、 2. 数据集搞不懂
        train_filenames = readlines(fpath.format("train")) # 训练数据集所在文件夹
        val_filenames = readlines(fpath.format("val")) # 测试数据集所在文件夹
        ...
        if self.num_pose_frames == 2:
            ...
            for f_i in self.opt.frame_ids[1:]:
                if f_i != "s":
                    ...
                    outputs[("axisangle", 0, f_i)] = axisangle  # 错误但不影响
                    outputs[("translation", 0, f_i)] = translation  # 错误但不影响
    ...
    def compute_losses(self, inputs, outputs):
        ...
        for scale in self.opt.scales:
            ...
            for frame_id in self.opt.frame_ids[1:]:
                ...
                reprojection_losses.append(self.compute_reprojection_loss(pred, target)) # 用-1和1重投影出的0计算reprojection_loss
