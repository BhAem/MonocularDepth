...
def evaluate(opt):
    ...
    if opt.ext_disp_to_eval is None:
        ...
        dataloader = DataLoader(dataset, batch_size=4, shuffle=False, num_workers=opt.num_workers, drop_last=False)

        encoder = networks.ResnetEncoder(opt.num_layers, 'scratch', use_aspp=opt.use_aspp)
        depth_decoder = networks.DepthDecoder(encoder.num_ch_enc,opt.att,opt.use_frelu)
...
if __name__ == "__main__":
    ...
    opt = options.parse()
    for i in range(0, 2):
        opt.load_weights_folder = './fj_best_640x192/models/weights_' + str(i) + '/'
        if i == 2:
            opt.load_weights_folder = './fj_best_640x192/models/weights_best/'
        print(i)
        evaluate(opt)
